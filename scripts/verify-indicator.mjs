/** 실제 학교 자료 없이 XLSX 등록→집계→내보내기→재열기 검증. */
...
if(!process.env.ADMIN_PASSWORD)throw Error('ADMIN_PASSWORD가 필요합니다.');
let cookie='',passed=0;
...
await data('/api/admin/login','POST',{password:process.env.ADMIN_PASSWORD});
const form=new FormData();form.set('file',new Blob([readFileSync('.verify-indicator-template.xlsx')]),'synthetic-2026.xlsx');
check((await req('/api/admin/templates/indicator-xlsx','POST',form)).status===200,'2026 합성 양식 등록');
...
check((await req('/api/export/indicator-xlsx?resultId='+r.id)).status===400,'대상·영역이 빠진 XLSX 제출 차단');
r=(await data('/api/results/aggregate','POST',{uploadIds:ids})).result;
const out=await req('/api/export/indicator-xlsx?resultId='+r.id);check(out.status===200,'완전한 집계의 XLSX 생성');
